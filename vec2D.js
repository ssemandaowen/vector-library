export default class Vec2D {
  ...
  // Instance method to rotate the vector by an angle (in radians)
  rotate(angle) {
  ...
  // Calculate the angle of the vector (in radians) with the x-axis
  angleWithXAxis() {
  ...
  // Reflect this vector across a specified vector
  reflectAcross(v) {
  ...
  // Get a new vector with rounded components
  round() {
    return new Vec2D(Math.round(this.x), Math.round(this.y));
  }

  // Scale the vector by different factors along x and y
  scale(sx, sy) {
    return new Vec2D(this.x * sx, this.y * sy);
  }

  // Get a unit vector pointing in the direction of the vector
  direction() {
    return this.mag() !== 0 ? this.norm() : new Vec2D();
  }

  // Create a vector representing a 2D wave at a specific phase
  ...
  // Translate the vector by a specified offset
  translate(dx, dy) {
  ...
  // Check if a point (vector) is inside a specified rectangular area
  isInsideRect(x1, y1, x2, y2) {
    return this.x >= x1 && this.x <= x2 && this.y >= y1 && this.y <= y2;
  }

  // Find the angle formed by three points (vectors)
  ...
  // Check if a vector is within a circular area
  isInsideCircle(center, radius) {
    return Vec2D.distanceBetween(this, center) <= radius;
  }

  // Rotate the vector around a specified pivot point
  ...
  // Create a vector with components rounded to a specified number of decimals
  ...
  // Interpolate smoothly between two vectors using cubic easing
  ...
  // Get the reflection vector given a surface normal
  reflect(normal) {
  ...
  // Create a vector that points to the mouse coordinates relative to a given HTML element
  static vectorFromMouse(event, element) {
  ...
  // Check if a vector is within a specified triangular region
  ...
  // Generate a vector representing a 2D Perlin noise field
  ...
  // Apply a 2D transformation matrix to the vector
  applyMatrix(matrix) {
  ...
  // Find the intersection point between two vectors (lines)
  ...
  // Find the projection of a vector onto a line defined by two points
  ...
  // Determine if a vector is on the left or right side of a directed line
